...
class MongoAdapter(AbstractDBAdapter):
    ...
    async def get_objects_from_db(self,
        ...
        collection = getattr(self.mongo_client, self.database_name).get_collection(collection_name)
        data = []
    ...
    @backoff.on_exception(backoff.expo, ConnectionError)
    async def get_object_from_db(self,
                                 model: Any,
                                 query: dict,
                                 collection_name: str):
    ...
    @backoff.on_exception(backoff.expo, ConnectionError)
    async def add_object_to_db(self,
                               obj: dict,
                               collection_name: str):
    ...
    @backoff.on_exception(backoff.expo, ConnectionError)
    async def update_object_from_db(self,
                                    query: dict,
                                    updated_fields: dict,
                                    collection_name: str):
