...
class BaseService:
    ...
    async def add_object(self, obj: Any):
        already_exists_object = await self.db_adapter.get_object_from_db(
            self.model,
            {'film_id': obj.film_id, 'user_login': obj.user_login},
            self.collection_name,
        )
        if already_exists_object:
            raise ObjectAlreadyExists()
        await self.db_adapter.add_object_to_db(
            obj.dict(exclude_none=True), self.collection_name
        )
    ...
    async def delete_object(self, **kwargs):
        ...
        await self.db_adapter.delete_object_from_db(kwargs, self.collection_name)
