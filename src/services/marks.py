...
class FilmsService(BaseService):
    async def get_average_rating(self, film_id: uuid.UUID):
        ...
        films = await self.db_adapter.get_objects_from_db(
            Mark, query, self.collection_name
        )
    ...
    async def update_mark(self, user_login: str, film_id: uuid.UUID, mark_value: int):
        mark = await self.db_adapter.get_object_from_db(
            Mark, {'film_id': film_id, 'user_login': user_login}, self.collection_name
        )
        if not mark:
            raise ObjectNotExists()
        await self.db_adapter.update_object_from_db(
            {'film_id': mark.film_id, 'user_login': mark.user_login},
            {'$set': {'mark': mark_value}},
...
def get_films_service(
    ...
    return FilmsService(
        db_adapter=db_adapter, model=Mark, collection_name=MONGO_MARK_COLLECTION_NAME
    )
